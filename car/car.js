...
define([
    ...
    function Car(position) {
        ...
        this.scene = TundraSDK.framework.scene;
    ...
    Car.prototype = {
        ...
        connectionEstablished: function() {
            ...
            this.scene.onEntityAction(this, this.onActionTriggered);
        },

        onActionTriggered: function(entityAction) {
            if (entityAction.name === "carCreated") {
                console.log("findMyCar");

                this.reservedCar = undefined;

                // Find a car that matches with the player
                this.serverCarCtrl = this.scene.entityByName("CarController");
                for (var i = 0; i < this.serverCarCtrl.dynamicComponent.cars.length; i++) {
                    var entityID = +this.serverCarCtrl.dynamicComponent.cars[i]; // Using the unary plus operator, it is possible to cast to a number.
                    var entity = this.scene.entityById(entityID);
                    if (!entity) {
                        console.log("entity not found: " + entityID);
                    } else if (entity.dynamicComponent.name === "Car" && entity.dynamicComponent.playerID == TundraSDK.framework.client.loginProperties.uniqueID) {
        ...
        update: function() {
            ...
            // if (TundraSDK.framework.client.isConnected()) {
        ...
        updateRemoteCarPhysijs: function(entID) {
            var entity = this.scene.entityById(entID);
            if (!entity || entity.dynamicComponent.name !== "Car" || entity.boxMesh === "loading" || entity.dynamicComponent.playerID == TundraSDK.framework.client.loginProperties.uniqueID) {
                return;
            }
            var ent = entity;
